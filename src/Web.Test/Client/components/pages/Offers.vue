...
export default {
    ...
    data(){
        return {
            ...
            formFields:[{key:'name', id:'OfferName', type:'text', label:'Offer', placeholder:"Enter offer name" }
                        ,{key:'description', id:'DescriptionName', type:'lng-text', label:'Description', placeholder:"Enter offer description" }],
            items:[],
            fields:[
    ...
    methods:{
      ...
      addItem(evt){
        evt.preventDefault();          
        axios.post('/api/offers/addoffer', { Name : this.newOffer.name, Description: this.newOffer.description} )
        this.refresh()
      ...
      refresh(){
        let self = this  
        axios.get('/api/offers/list?page=0&size='+((page+1)*size)+'&search='+searching).then(r=>{          
            if(r.data.success)
            {  
